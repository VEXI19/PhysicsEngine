...
class Gui(QtWidgets.QWidget):
    ...
    def setupUI(self):
        ...
        self.view.setWindowTitle('3D Rocket Trajectory')
        self.view.setCameraPosition(distance=50)  # Adjusted for better view
        grid = gl.GLGridItem()
        grid.setSize(20, 20)
        ...
        # Data graphs
        self.velocity_curve_dict = self.add_plot(graph_layout, "Velocity", "Time", "Velocity", ["r", "g", "b"], ["Velocity X", "Velocity Y", "Velocity Z"])
        self.acceleration_curve_dict = self.add_plot(graph_layout, "Acceleration", "Time", "Acceleration", ["r", "g", "b"], ["Acceleration X", "Acceleration Y", "Acceleration Z"])
        self.altitude_curve = self.add_plot(graph_layout, "Altitude", "Time", "Altitude", "r", "Altitude")

        # Timer for 3D animation and 2D plot sync
        self._timeline = TimeLine(loopCount=0, interval=int(1000 * self.time_step))
        self._timeline.setFrameRange(0, self.data_points - 1)
        ...
        self.slider.setRange(0, self.data_points - 1)  # Frame range
    ...
    def add_plot(self, parent, title: str, x_label: str, y_label: str, pen: str | list[str], name: str | list[str], legend: bool = True):
    ...
    def read_data(self):
        ...
        with open(self.file_path, "r") as file:
            ...
            self.positions = np.column_stack((data['pos_x'], data['pos_y'], data['pos_z']))
            self.velocities = [data['vel_x'], data['vel_y'], data['vel_z']]
            self.accelerations = [data['acc_x'], data['acc_y'], data['acc_z']]
            self.data_points = len(self.positions)
    ...
    def update_trajectory(self, i):
        ...
        self.rocket_point.setData(pos=np.array([self.positions[i]]))  # Update rocket position
        self.trajectory_line.setData(pos=self.positions[:i + 1])  # Update trajectory

        # Update velocity and acceleration graphs
        self.velocity_curve_dict["Velocity X"].setData(self.velocities[0][:i + 1])
        self.velocity_curve_dict["Velocity Y"].setData(self.velocities[1][:i + 1])
        self.velocity_curve_dict["Velocity Z"].setData(self.velocities[2][:i + 1])

        self.acceleration_curve_dict["Acceleration X"].setData(self.accelerations[0][:i + 1])
        self.acceleration_curve_dict["Acceleration Y"].setData(self.accelerations[1][:i + 1])
        self.acceleration_curve_dict["Acceleration Z"].setData(self.accelerations[2][:i + 1])

        self.altitude_curve.setData(self.positions[:i + 1, 2])
...
if __name__ == '__main__':
    ...
    gui = Gui("./Simulations/Rocket_2/simulation_2024-11-27_22-34-30.csv")
