...
opartego na algorytmach samouczących, zdolnego do przeprowadzenia
stabilnego, pionowego lotu rakietą o naturalnie niestabilnej charakterystyce.
W celu spełnienia tych założeń konieczne było stworzenie środowiska,
pozwalającego na wielokrotne prowadzenie symulacji lotu rakiety i trenowanie
algorytmu sterującego.
Stworzenie silnika fizycznego spełniającego te wymagania było pośrednim celem
niniejszej pracy.\\
W celu osiągnięcia założonego celu, poczynione zostały pewne założenia,
które precyzują zakres pracy. Są to założenia szczegółowo opisane w
rozdziałach \ref{sec:zalozenia-projektowe.} oraz
\ref{sec:założenia-projektowe-2.}.\\
Zakres pracy obejmuje stworzenie silnika fizycznego symulacji lotów
rakietowych jako pierwszego modułu projektu, a także stworzenie i
wytrenowanie modelu sterującego rakietą jako drugiego modułu projektu. Oba
te moduły są ze sobą ściśle związane, a ich integracja jest kluczowa dla
powodzenia projektu i stanowi, kolejny z elementów zakresu pracy.\\
Praca ta skupia się wyłącznie na stworzeniu symulacji
lotów,a  jej
projektowa część jest związana z informatyczną i programistyczną
sferą zagadnienia.
Do zakresu niniejszej pracy inżynierskiej nie należą więc tematy
związane z konstrukcją fizycznych rakiet ani z budową sprzętu do pomiarów
lotów rakietowych czy testowanie skuteczności algorytmów sterujących w lotach
fizycznych rakiet.
\section{Struktura pracy}

Niniejsza praca inżynierska powstała w wyniku kolaboracji dwóch
współtwórców, dlatego też główna jej część została podzielona na dwa odrębne
moduły:
\begin{itemize}
    \item Moduł I: System symulacji lotów rakietowych. Autor: Jakub
    Kindlik;
...
pracy. Te części niniejszej pracy inżynierskiej są wspólne dla obu modułów i
zostały napisane wspólnie przez obu autorów.

\section{Użyte technologie}
Poszczególne biblioteki i narzędzia wykorzystane przy tworzeniu każdego z
modułów pracy zostały przedstawione i opisane odpowiednich
rozdziałach pracy, to jest odpowiednio w rozdziale
\ref{sec:uzyte-technologie.} w module I, oraz w rozdziale
\ref{sec:użyte-technologie-2.} w module II. Jednakże niektóre technologie
oraz narzędzia zostały wykorzystane przy tworzeniu obu modułów projektu,
wchodzących w skład niniejszej pracy inżynierskiej.

\begin{itemize}
    \item \textbf{Git \cite{chacon2014pro} oraz Github \cite{GitHub}} -
    narzędzia
    wykorzystane do kontroli
    ...
    \item \textbf{Python 3 \cite{python3docs}} - język programowania, w
    którym napisane
    ...
    \item \textbf{PyCharm Professional \cite{pycharm}}  - środowisko
    programistyczne
...
\begin{itemize}
    \item \textbf{LaTex \cite{mittelbach2004latex}} - system składu tekstu
    ...
    \item \textbf{SGGW-thesis \cite{chmiel2024sggwthesislatex}} - biblioteka
    do tworzenia pracy
    inżynierskiej
...
Dobra znajomość dostępnych na rynku rozwiązań pozwala na wybór najlepszego
narzędzia do konkretnego projektu, a także na zrozumienie specyfiki
symulacji lotów rakietowych oraz algorytmów sterujących, co jest kluczowe
dla skutecznego rozwoju systemów sterowania rakietami. Dzięki zapoznaniu się
z obecnymi rozwiązaniami możliwe było stworzenie własnego, dedykowanego
narzędzia do symulacji lotów rakietowych oraz algorytmu sterującego.


\subsection{System symulacji lotów rakietowych.}
...
\begin{itemize}
    \item \textbf{Symulink \cite{matlab2019simulink}.} \\Bardzo zaawansowane
    narzędzie do
    ...
    \item \textbf{ANSYS Fluent \cite{ansys2020fluent}.}
    ...
    \item \textbf{OpenFOAM \cite{openfoam2020}.}
    ...
    Główną zaletą OpenFOAM jest jego elastyczność i dostępność –
    oprogramowanie jest darmowe i można je dostosować do indywidualnych
    potrzeb poprzez tworzenie własnych modułów i skryptów. Jednak jego
    obsługa wymaga zaawansowanej wiedzy technicznej oraz umiejętności pracy
    w środowisku tekstowym, co może być barierą dla początkujących
    użytkowników\cite{openfoam2023}. \\
...
\end{itemize}
Dedykowane środowiska i systemy, służące do symulowania lotów
rakietowych:

\begin{itemize}

    \item \textbf{OpenRocket \cite{openrocket}.}
    ...
    inżynierskiej\cite{niskanen2013openrocket}.


    \item \textbf{RockSim \cite{rocksim}.}
    ...
    \item \textbf{RAS Aero \cite{rasaero}.}
...
\begin{itemize}
    \item \textbf{TensorFlow \cite{DBLP:journals/corr/AbadiBCCDDDGIIK16}.} \\
...
\begin{itemize}
    ...
    \cite{mathworks_aerospace_blockset}.} \\
    ...
    \item \textbf{Microsoft AirSim \cite{airsim}.} \\
    ...
    \item \textbf{Gazebo \cite{gazebo_ros} + ROS \cite{ros_gazebo}.} \\
...
sztucznej inteligencji. Narzędzia te są w większości bardzo zaawansowane i
...
czy analiza trajektorii lotu. Użycie dokładnych symulacji i trenowanie
modeli, za pomocą dobrze dobranych algorytmów umożliwia osiągnięcie bardzo
dobrych wyników w sterowaniu lotem rakiety.\\
\\ W niniejszej pracy inżynierskiej wykorzystane zostały własne implementacje silnika fizycznego oraz algorytmu sterującego,
co pozwoliło na pełną kontrolę nad procesem tworzenia i testowania systemu sterującego rakietą.
Efektem tego jest także uproszczony model fizyczny, o czym mowa jest w kolejnych rozdziałach pracy.
Rozwiązanie prezentowane w tej pracy odbiega od profesjonalnych narzędzi pod względem skomplikowania i dokładności,
jednak pozwala na zrozumienie sposobu działania symulacji lotu rakiety oraz algorytmów sterujących i jest
dobrą bazą do rozwoju bardziej zaawansowanych systemów w przyszłości.